...
ggml_tensor * delta_net::build_qkv(ggml_context * ctx0, ggml_tensor * state_storage, ggml_tensor * ssm_conv1d,
    ...
    if (state_dst->type == GGML_TYPE_F32) {
        ...
        // FP16 state storage (Phase 1c): concat kernel only writes F32, so
        // concat out-of-place and let ggml_cpy cast into the F16 cache view
        ggml_tensor * new_state_flat = ggml_concat(ctx0, new_conv_flat, new_ssm_flat, 0);
        state_cpy = ggml_cpy(ctx0, new_state_flat, state_dst);
    }
